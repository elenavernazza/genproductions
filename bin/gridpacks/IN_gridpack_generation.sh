      ...
      if [ -e $CARDSDIR/${name}_extramodels.dat ]; then
        ...
        do
          ...
          if [[ $model = *[!\ ]* ]]; then
            ...
            wget --no-check-certificate http://stash.osgconnect.net/+elenavernazza/$model
#            cp /afs/cern.ch/user/e/evernazz/D6EFTStudies/madgraph_model/$model ./	
            cd models
            if [[ $model == *".zip"* ]]; then
              unzip ../$model
            elif [[ $model == *".tgz"* ]]; then
...
if [ -n "$5" ]
  then
    ...
    scram_arch=slc7_amd64_gcc700 #slc6_amd64_gcc481
fi
...
if [ -n "$6" ]
  then
    ...
    cmssw_version=CMSSW_10_6_0 #CMSSW_10_2_13 #CMSSW_7_1_30
fi
