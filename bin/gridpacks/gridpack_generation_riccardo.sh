...
make_gridpack () {
    ...
    if [ ! -d ${GEN_FOLDER}/${name}_gridpack ]; then
      ...
      #tar xzf ${MG}
      #rm "$MG"
      cp -r /afs/cern.ch/user/r/ribrusa/public/MG5_aMC_v2_6_5 ./
      ...
      if [ -e $CARDSDIR/${name}_extramodels.dat ]; then
        ...
        do
          ...
          if [[ $model = *[!\ ]* ]]; then
            ...
            cp /afs/cern.ch/user/r/ribrusa/public/$model ./
            cd models
            if [[ $model == *".zip"* ]]; then
              unzip ../$model
            elif [[ $model == *".tgz"* ]]; then
    ...
    #echo "Modifying cluster.py from the following folder:"
    #echo `pwd`
    #for fil in `find ./ -name *make_opts*` ; do sed -i "s/FFLAGS= -O -w -fbounds-check -fPIC/FFLAGS= -O -w -fbounds-check -fPIC -mcmodel=medium/g" $fil ;done
    #for fil in `find ./ -name cluster.py` ; do sed -i "s/raise ClusterManagmentError('Some Jobs are in a Hold/#raise ClusterManagmentError('Some Jobs are in a Hold/g" $fil ;done
    #read
...
if [ -n "$5" ]
  then
    ...
    scram_arch=slc6_amd64_gcc630 #slc7_amd64_gcc700 #slc6_amd64_gcc630 #slc6_amd64_gcc481
fi
...
if [ -n "$6" ]
  then
    ...
    cmssw_version=CMSSW_9_3_16 #CMSSW_10_6_0 #CMSSW_9_3_16 #CMSSW_7_1_30
fi
